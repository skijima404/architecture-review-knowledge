...
FAILURE_MODEL_FOLDERS = (
    "knowledge/curated/failure-model/root-causes",
    "knowledge/curated/failure-model/symptoms",
...
def load_existing_relations(repo_root: Path) -> dict[str, dict]:
    ...
    for dir_name in FAILURE_MODEL_FOLDERS:
...
def extract_generated_relations(yaml_data: dict, miro_to_node: dict) -> dict[str, dict]:
    ...
    for node_type in ["success_criteria", "symptom", "root_cause"]:
        for node in yaml_data.get(node_type, []):
            ...
            for edge_type in RELATION_FIELDS:
...
def main() -> int:
    ...
    yaml_path = tool_root / args.yaml
    repo_root = Path(args.repo_root) if args.repo_root else tool_root.parent.parent
