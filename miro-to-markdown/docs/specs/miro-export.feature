Feature: Miro Board Export
  ...
  Scenario: Miro node matches existing Markdown file by title
    Given an existing Markdown file "root_cause/rc-006.md" with title "Unrealistic transition architecture"
  ...
  Scenario: Export uses repository root when base_path is ".."
    ...
    And existing files in repository root success_criteria/, symptom/, root_cause/
    When I run the export CLI
